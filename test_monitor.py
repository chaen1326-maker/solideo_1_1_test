#!/usr/bin/env python3
"""
시스템 리소스 모니터링 시스템 - 테스트 프로그램
30초 동안 시스템 리소스를 모니터링하고 PDF 보고서를 생성합니다.
"""
...
def main():
    """메인 함수"""
    # 모니터링 시간 설정 (초)
    MONITORING_DURATION = 30  # 30초 (테스트용)
    COLLECTION_INTERVAL = 1.0  # 1초마다 수집

    print("=" * 80)
    print(" " * 20 + "시스템 리소스 모니터링 시스템 (테스트)")
    print("=" * 80)
    print()
    print(f"모니터링 시간: {MONITORING_DURATION}초")
    ...
    print("\nPDF 보고서를 생성하는 중...")
    try:
        pdf_generator = PDFReportGenerator("test_system_report.pdf")
        pdf_generator.generate_report(summary, all_data, graphs)
        print("  ✓ PDF 보고서 생성 완료: test_system_report.pdf")
    except Exception as e:
